...
def clear_log_meta_model(output_file_name, run_num, exp_dir):
    """
    Clear the log, meta, and model files generated by this run
    """
    run_name = f"{output_file_name} {run_num}"
    try:
        os.remove(f"{exp_dir}/{run_name}_log")
    except OSError:
        pass
    try:
        os.remove(f"{exp_dir}/{run_name}_meta")
    except OSError:
        pass
    try:
        os.remove(f"{exp_dir}/{run_name}_model")
    except OSError:
        pass


def calculate_average(output_file_name, run_count, exp_dir):
    csv_path = f"{exp_dir}/{output_file_name}.csv"
...
def print_output_to_file(output_file_name, run_num, exp_dir):
    run_name = f"{output_file_name} {run_num}"

    log_name = f"{exp_dir}/{run_name}_log"
    file = open(log_name, 'rb')

    outfile = open(f"{exp_dir}/{run_name}.txt", 'a')
